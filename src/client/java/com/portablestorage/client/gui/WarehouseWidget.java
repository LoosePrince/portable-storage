...
import com.portablestorage.network.C2SClickWarehouseSlotPayload;
...
public class WarehouseWidget {
    private static final Identifier WAREHOUSE_SLOT_TEXTURE = com.portablestorage.PortableStorage.id("textures/gui/slot.png");
    ...
    public WarehouseWidget(AbstractContainerScreen<?> screen) {
        ...
        this.warehouse = snapshot == null ? new PlayerWarehouse(java.util.UUID.randomUUID(), __ -> {}) : snapshot;
    }
    ...
    private void updateSlotPositions() {
        ...
        for (Slot slot : screen.getMenu().slots) {
            if (slot instanceof com.portablestorage.upgrade.UpgradeSlot upgradeSlot) {
                ...
                int upgradeYBase = WarehouseConstants.getWarehouseYOffset(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded()) + WarehouseConstants.UPGRADE_SLOT_RELATIVE_Y;
                ...
                ((com.portablestorage.mixin.accessor.SlotAccessor) slot).setY(WarehouseConstants.getSlotLogicY(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded()) + row * WarehouseConstants.SLOT_SIZE);
            }
        }
    }
    ...
    public void adjustScreenPosition(int rows) {
        ...
        if (ModConfig.offsetInventory && !warehouse.isFolded() && (screen instanceof InventoryScreen
            ...
            if (pos.isVertical()) {
                ...
                if (pos == StoragePosition.BOTTOM) yOffset = -yOffset;
            } else {
                xOffset = (warehouseWidth + spacingX) / 2;
                if (pos == StoragePosition.RIGHT) xOffset = -xOffset;
            }
        }
    ...
    private void initSearchBox() {
        ...
        this.searchBox.setHint(Component.translatable("gui.portablestorage.search").withStyle(style -> style.withColor(0x666666)));
    ...
    public void renderBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        int y = topPos + WarehouseConstants.getWarehouseYOffset(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded());
    ...
    private FoldButtonLayout getFoldButtonLayout(int leftPos, int topPos, int imageHeight) {
        if (screen instanceof com.portablestorage.screen.ToolWarehouseScreen || screen instanceof com.portablestorage.screen.BoundBarrelScreen) {
        ...
        if (isContainerInterface()) {
            ...
            int warehouseY = topPos + WarehouseConstants.getWarehouseYOffset(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded());
            return new FoldButtonLayout(warehouseX + WarehouseConstants.getSidebarXOffset(),
                    warehouseY + WarehouseConstants.getSidebarYOffset(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded()), true);
        }
        return new FoldButtonLayout(leftPos + WarehouseConstants.FOLD_BUTTON_X_OFFSET, topPos + WarehouseConstants.FOLD_BUTTON_Y_OFFSET, false);
    }
    ...
    private FoldButtonLayout getInventoryTopRightFoldButtonLayout(int leftPos, int topPos) {
        ...
        for (Slot slot : screen.getMenu().slots) {
            if (slot.container == Minecraft.getInstance().player.getInventory() && slot.getContainerSlot() >= 9 && slot.getContainerSlot() < 36) {
    ...
    private void renderCraftingBg(GuiGraphicsExtractor graphics) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, WAREHOUSE_SLOT_TEXTURE, cx + col * 18, cy + row * 18,
    ...
    public void flushDebouncedSearchPacket() {
        if (pendingSearchText != null && System.currentTimeMillis() - lastSearchUpdateTime > SEARCH_PACKET_DEBOUNCE_MS) {
    ...
    public boolean tryConsumeCharForSearch(CharacterEvent event) {
        ...
        if (this.searchBox == null || !this.searchBox.isVisible() || !this.searchBox.active || !this.searchBox.isFocused()) return false;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // Upgrade Slot Interactions (Right or Middle click)
        ...
        // Pinned Toggle (Middle Click)
        if (!warehouse.isFolded() && button == 2) {
            int baseX = screenAccessor.portablestorage$getLeftPos() + WarehouseConstants.getSlotLogicX();
            int baseY = screenAccessor.portablestorage$getTopPos() + WarehouseConstants.getSlotLogicY(warehouse.getVisibleRows(), screenAccessor.portablestorage$getImageHeight(), warehouse.isFolded());
        ...
        if (clickedSlot != null) {
            ...
            if (isWarehouseSlot && !isUpgradeSlot && button == 0) {
                ...
                if (customData != null && customData.copyTag().getBoolean(WarehouseConstants.SMART_COLLAPSE_TAG).orElse(false)) {
                    if (warehouse.isSmartCollapse() && warehouse.getSearchText().isEmpty()) {
            ...
            // Normal Left/Right click on Warehouse or Upgrade slot
            if ((isWarehouseSlot || isUpgradeSlot) && (button == 0 || button == 1)) {
                ...
                if (isUpgradeSlot) {
                    ...
                    if (!cursorStack.isEmpty()) {
                        ...
                        screen.getMenu().setCarried(remaining);
                    } else {
                        int amount = (button == 1) ? 1 : 64;
                        ItemStack taken = WarehouseManager.removeItem(warehouse, clickedSlot.getContainerSlot(), amount, false);
                        screen.getMenu().setCarried(taken);
                    }
                }

                ClientPlayNetworking.send(new C2SClickWarehouseSlotPayload(clickedSlot.getContainerSlot(), button, isUpgradeSlot));
                return true;
            }
        }
        ...
        if (shouldShow() && !warehouse.isFolded()) {
            ...
            if (minecraft.options.keyDrop.matches(event)) {
                ...
                if (hSlot != null && hSlot.container instanceof PlayerWarehouse && hSlot.hasItem()) {
                    ...
                    ClientPlayNetworking.send(new C2SDropWarehouseItemPayload(hSlot.getContainerSlot(), dropFullStack));
                    return true;
                }
            }
        }
    ...
    private boolean isMouseOverSearchClear(double mouseX, double mouseY) {
        if (searchBox == null || !searchBox.isVisible() || !searchBox.active) return false;
        ...
        int wy = topPos + WarehouseConstants.getWarehouseYOffset(warehouse.getVisibleRows(), imageHeight, warehouse.isFolded());
        int clearLeft = wx + WarehouseConstants.getSearchBoxXOffset() + WarehouseConstants.SEARCH_BOX_WIDTH - WarehouseConstants.SEARCH_BOX_INNER_OFFSET - WarehouseConstants.SEARCH_CLEAR_ZONE_WIDTH;
        ...
        return mouseX >= clearLeft && mouseX < clearLeft + WarehouseConstants.SEARCH_CLEAR_ZONE_WIDTH && mouseY >= clearTop && mouseY < clearTop + clearH;
    }

    private void clearSearch() {
        if (searchBox != null) searchBox.setValue("");
        else warehouse.setSearchText("");
        pendingSearchText = null;
