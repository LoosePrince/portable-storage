...
public class BarrelUpgrade extends UpgradeType {
    public static final Identifier ID = PortableStorage.id("barrel");
    ...
    public List<Component> getTooltip(PlayerWarehouse warehouse, ItemStack stack) {
        ...
        if (customData != null) {
            ...
            String ownerId = tag.getString("owner").orElse("");
            if (!ownerId.isEmpty()) {
                String ownerName = tag.getString("ownerName").orElse("");
                if (ownerName.isEmpty()) {
                    ownerName = "Unknown";
                }
            tooltips.add(Component.literal(" "));
            tooltips.add(Component.translatable("upgrade.portablestorage.barrel.bound_to", ownerName)
    ...
    public void onInstall(PlayerWarehouse warehouse, ItemStack stack) {
        if (stack.is(Items.BARREL)) {
            ...
            CustomData.update(DataComponents.CUSTOM_DATA, boundStack, tag -> {
                tag.putString("owner", warehouse.getOwnerUuid().toString());
        ...
        if (stack.is(ModItems.BOUND_BARREL)) {
            ...
            if (customData != null) {
                var tag = customData.copyTag();
                    String ownerId = tag.getString("owner").orElse("");
                    if (!ownerId.isEmpty()) {
                        java.util.UUID ownerUuid = java.util.UUID.fromString(ownerId);
                        if (ownerUuid.equals(player.getUUID())) {
                            String currentName = tag.getString("ownerName").orElse("");
