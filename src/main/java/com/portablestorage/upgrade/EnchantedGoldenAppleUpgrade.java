...
public class EnchantedGoldenAppleUpgrade extends UpgradeType {
    public static final Identifier ID = PortableStorage.id("enchanted_golden_apple");
    ...
    public List<Component> getTooltip(PlayerWarehouse warehouse, ItemStack stack) {
        ...
        tooltips.add(Component.translatable("upgrade.portablestorage.enchanted_golden_apple.desc")
                .withStyle(ChatFormatting.GRAY));

        CustomData data = stack.get(DataComponents.CUSTOM_DATA);
        int threshold = 0;
        if (data != null) {
            threshold = data.copyTag().getInt(TAG_THRESHOLD).orElse(0);
        }

        tooltips.add(Component.literal(" "));
        Component statusText = threshold == 0
                ? Component.translatable("gui.portablestorage.off").withStyle(ChatFormatting.RED)
                : Component.literal(String.valueOf(threshold)).withStyle(ChatFormatting.GREEN);
        tooltips.add(Component.translatable("upgrade.portablestorage.enchanted_golden_apple.threshold", statusText)
                .withStyle(ChatFormatting.YELLOW));

        tooltips.add(Component.literal(" "));
        tooltips.add(Component.translatable("upgrade.portablestorage.enchanted_golden_apple.toggle_hint")
                .withStyle(ChatFormatting.DARK_GRAY));
        tooltips.add(Component.translatable("upgrade.portablestorage.enchanted_golden_apple.filter_hint")
                .withStyle(ChatFormatting.DARK_GRAY));
    ...
    public void onRightClick(PlayerWarehouse warehouse, Player player) {
        ...
        if (!stack.isEmpty()) {
            CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> {
                int current = tag.getInt(TAG_THRESHOLD).orElse(0);
                int nextIndex = 0;
    ...
    public void onMiddleClick(PlayerWarehouse warehouse, Player player) {
        if (player instanceof ServerPlayer serverPlayer) {
            ServerPlayNetworking.send(serverPlayer,
                    new S2COpenFoodFilterPayload(warehouse.getFoodFilters(), warehouse.isFoodFilterBlacklist()));
        }
    }
    ...
    private void autoEat(PlayerWarehouse warehouse, ServerPlayer player) {
        ...
        for (WarehouseEntry entry : storage) {
            ...
            if (food != null) {
                String itemId = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(foodStack.getItem())
                        .toString();
        ...
        if (bestFood != null) {
            ...
            // 模拟进食：手动应用饱食度
            FoodProperties foodProps = toEat.get(DataComponents.FOOD);
            if (foodProps != null) {
    ...
    private int getThreshold(ItemStack stack) {
        CustomData data = stack.get(DataComponents.CUSTOM_DATA);
        if (data != null) {
            return data.copyTag().getInt(TAG_THRESHOLD).orElse(0);
        }
        return 0;
    }
}
