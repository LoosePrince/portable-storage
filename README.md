...
> 这意味着，我将无法保证模组的可用性、兼容性、稳定性，请确保你能接受并有能力保护好数据再使用本模组。
...
- **潮涌核心**：根据仓库中海晶系方块数量获得效果（海晶石、暗海晶石、海晶石砖、海晶灯）；
  - 达 16 个且接触水/雨时：每秒获得 13 秒潮涌能量；
  - 达 42 个可激活攻击模式；右键槽位在默认/攻击模式间切换；
  - 攻击模式：每 2 秒对周围 8 格内单个敌对生物造成 4 点玩家伤害并击退（排除猪灵、猪灵蛮兵、疣猪兽）；
  - 服务端可配置“启用潮涌核心升级”，关闭后该槽位不显示且效果无效。
- **附魔之瓶**：在仓库显示“瓶装经验”；
...
- **Minecraft 版本**：
  ...
  - 1.21：1.x版本 √ 2.x版本 √ 2.0.9版本后不再更新
  - 1.21.1：1.x版本 √ 、2.x版本 √ 2.0.9版本后不再更新
  - 1.21.11：实现测试中
