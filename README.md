...
仓库界面左侧有升级槽位，可以放入特定物品以解锁仓库的额外功能，右侧是仓库设置面板。部分功能也提供可选快捷键，默认未绑定，不会占用玩家现有按键。
...
- **工具**：允许放入斧、镐、锄、锹作为升级；
  - 右键工具升级槽位打开 3x3 工具容器；
  - 工具容器中的物品属于仓库存储，会占用仓库种类上限；
  - 工具容器中的物品不会显示在主仓库列表、搜索结果和普通排序结果中；
  - 可在按键设置中绑定“快捷工具调用”，按住后通过 `1-9` 或鼠标滚轮选择工具槽，松开按键后由服务端执行换手；
  - 快捷工具 HUD 最左侧为空白取消格，默认选中，按 `0` 或滚轮可切回该格，松开按键不进行交换或存取；
  - 选中空工具槽时，松开快捷键会尝试把手中物品放入该工具槽，仍遵循仓库存储条件、容量上限和 NBT 大小限制。
...
- **快捷展开/折叠仓库**：可在按键设置中绑定，默认未绑定；在可显示仓库 UI 的背包或容器界面内触发，用于切换仓库展开状态；
- **关闭界面时自动折叠仓库**：可在客户端配置中开启，默认关闭；开启后关闭背包或容器界面时自动折叠仓库；
- **静态模式**：按住 `Ctrl` 时进入静态模式，存取只更新数量，不重排当前列表；数量变为 0 的条目会暂时保留为 0，松开 `Ctrl` 或关闭界面后恢复正常显示；
- **快捷工具调用**：安装工具升级并绑定按键后可用；按住快捷键显示工具 HUD，松开快捷键时才执行选择结果。

### 其它机制

- 流体储存：通过快速存取将流体存入仓库，支持岩浆、水、牛奶；
- 弓和弩的箭矢支持从仓库扣除；
- 在仓库中对物品中键可以将其设为收藏和取消收藏，收藏物品会排在其它物品前面；普通物品、流体储量和瓶装经验都支持收藏状态；
- 拼音搜索：可以首字母；
...
- 快捷键：可以去游戏设置中配置快速打开仓库合成界面、展开/折叠仓库、快捷工具调用的快捷按键，默认均未绑定。
...
- 感谢B站用户 [取的名字花半个小时](https://space.bilibili.com/1338918639) 提出的快捷工具调用想法和其它快捷键想法
